...
$elem["nbd-server/port"]["descriptionde"]="TCP-Port für Server Nummer ${number}:
 Bitte geben Sie den TCP-Port an, den diese Instanz des NBD-Servers für Rückwärtskompatibilität mit älteren Clients benutzen wird.
";
$elem["nbd-server/port"]["descriptionfr"]="Port TCP du serveur numéro ${number} :
 Veuillez indiquer le port TCP où cette instance du serveur NBD sera à l'écoute, pour compatibilité avec des clients plus anciens.
";
...
$elem["nbd-server/convert"]["descriptionde"]="Im alten Format vorliegende NBD-Server-Konfiguration konvertieren?
 Eine NBD-Server-Konfigurationsdatei vor 2.9 wurde auf diesem System gefunden. Das aktuelle NBD-Server-Paket unterstützt diese Datei nicht länger und wird nicht funktionieren, falls sie so beibehalten wird, wie sie ist.
 .
 Falls Sie diese Option wählen, wird das System eine Konfiguration im neuen Format erzeugen, die auf der Konfigurationsdatei im alten Format basiert, welche entfernt wird. Andernfalls werden Konfigurationsfragen gestellt und das System erzeugt eine neue Konfigurationsdatei.
 .
 Falls bereits eine Konfigurationsdatei im neuen Format existiert und Sie diese Option wählen, wird in Kürze wie gewohnt die Abfrage »modified configuration file« erfolgen.
";
...
$elem["nbd-server/useports"]["type"]="boolean";
$elem["nbd-server/useports"]["description"]="Support port-based exports?
...
";
$elem["nbd-server/useports"]["descriptionde"]="portbasierte Exporte unterstützen?
 Seit Version 2.9.18 unterstützt NBD-Server namens- anstatt portbasierte Exporte. Obwohl dies allgemein wünschenswert ist, müssen Sie möglicherweise ein System unterstützen, auf dem eine ältere Version von NBD-Client ausgeführt wird oder Sie könnten sich eine Menge Ärger einhandeln, um Ihre alten Konfigurationen auf namensbasierte Exporte zu migrieren. In diesen beiden Fällen sollten Sie diese Option akzeptieren.
 .
 Alle anderen sollten diese Option auf der Standardeinstellung belassen, die NBD-Server veranlasst, nur namensbasierte Exporte zu unterstützen.
";
$elem["nbd-server/useports"]["descriptionfr"]="Faut-il gérer les exports avec numéros de ports ?
 Depuis la version 2.9.18, nbd-server gère les exports nommés à la place des exports basés sur des numéros de ports. Bien que l'utilisation d'exports nommés soit préférable, vous pouvez avoir besoin de compatibilité avec des systèmes plus anciens qui utilisent une version de nbd-client qui ne les gère pas. Vous pouvez également préférer ne pas avoir à mettre à jour les configurations plus anciennes afin de migrer vers des exports nommés. Dans ces cas, vous devriez alors choisir cette option.
 .
 Dans le cas contraire, il sera suffisant de ne gérer que les exports nommés (ce qui est le mode de fonctionnement par défaut).
";
$elem["nbd-server/useports"]["default"]="false";
