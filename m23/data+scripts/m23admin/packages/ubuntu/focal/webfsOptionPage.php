...
";
$elem["webfsd/web_root"]["default"]="/var/www/html";
...
$elem["webfsd/web_index"]["description"]="Directory index filename:
 ...
 of a following dialog (\"Extra options to include\").
";
$elem["webfsd/web_index"]["descriptionde"]="Name der Indexdatei für Verzeichnisse:
 Falls Webfsd eine Anfrage für ein Verzeichnis erhält, kann er wahlweise nach einer Indexdatei suchen, die an den Client gesendet werden soll. Gebräuchliche Namen sind »index.html« und »default.html«. Wenn Sie dies leer lassen, wird Webfsd überhaupt keine Datei als Verzeichnisindex ansehen.
 .
 Da kein Index existiert, wird Webfsd im Normalfall eine Übersicht für jedes angeforderte Verzeichnis erzeugen. Wenn Sie eine solche erzeugte Verzeichnisübersicht allerdings als Sicherheitsrisiko betrachten, können Sie diese Übersicht auch deaktivieren, indem Sie den Server mit der Option »-j« laufen lassen. Diese Option kann in »/etc/webfsd.conf« zur Variable »web_extras« hinzugefügt werden, entweder manuell oder über den folgenden Dialog (»Zusätzliche Optionen«).
";
$elem["webfsd/web_index"]["descriptionfr"]="Fichier contenant les index des répertoires :
 Lorsque webfsd reçoit une requête sur un répertoire, il peut de manière optionnelle rechercher un fichier d'index à envoyer au client. Les noms usuels pour de tels fichiers d'index sont « index.html » et « default.html ». Si ce champ est laissé vide, webfs ne considérera aucun fichier comme un index de répertoire.
 .
 En l'absence d'index, webfs construit une liste de chaque répertoire accédé. Cependant, si vous considérez cela comme un risque pour la sécurité, la construction de ces listes peut être désactivée en lançant le serveur avec l'option « -j ». Vous pouvez ajouter cette option à la variable « web_extras »,soit directement dans le fichier /etc/webfsd.conf, soit à l’aidede l'option de configuration qui va vous être demandée(« Options supplémentaires à utiliser »).
";
...
$elem["webfsd/web_extras"]["descriptionde"]="Zusätzliche Optionen:
 ...
 .
 Dies ist speziell der richtige Zeitpunkt, die Option »-j« einzufügen, wodurch die Erzeugung von jeglicher Verzeichnisübersicht verhindert wird.
 .
...
$elem["webfsd/web_extras"]["descriptionfr"]="Options supplémentaires à utiliser :
 ...
 .
 Plus particulièrement, vous pouvez ajouter ici l'option « -j » afin d'éviter toute création de liste de fichiers dans les répertoires.
 .
