...
";
$elem["qpsmtpd/startup_enabled"]["descriptionde"]="Qpsmtpd beim Systemstart aktivieren?
 ...
 .
 Bevor Sie Qpsmtpd aktivieren, müssen Sie zuerst Ihren lokalen MTA so konfigurieren, dass er sich mindestens auf einer Schnittstelle nicht an den SMTP-TCP-Port bindet. Der häufigste Ansatz ist, Ihren MTA an der Loopback-Schnittstelle (127.0.0.1) auf Anfragen warten zu lassen, während Qpsmtpd auf der externen Schnittstelle wartet. Anleitungen, wie verbreitete MTAs so konfiguriert werden, dass sie mit Qpsmtpd zusammenarbeiten, können nach der Installation in /usr/share/doc/qpsmtpd/README.Debian gefunden werden.
 .
...
$elem["qpsmtpd/server_type"]["type"]="select";
$elem["qpsmtpd/server_type"]["choices"][1]="forkserver";
$elem["qpsmtpd/server_type"]["description"]="Qpsmtpd server type:
...
";
$elem["qpsmtpd/server_type"]["descriptionde"]="Qpsmtpd-Servertyp:
 Qpsmtpd unterstützt zwei Prozessmodelle für den Umgang mit Verbindungen. Das Modell »forkserver« verwendet im Leerlauf einen einzelnen Prozess und erzeugt neue Prozesse, um Verbindungen zu bedienen. Dies verbraucht weniger Speicher, reduziert aber leicht den Serverdurchsatz. Das Modell »prefork« hält ein Reservoir an Prozessen im Leerlauf vor, um neue Verbindungen zu bedienen. Damit wird eine leicht bessere Leistung auf Kosten von mehr Speicher ermöglicht.
";
$elem["qpsmtpd/server_type"]["descriptionfr"]="Type de serveur qpsmtpd :
 Le démon qsmtpd propose deux méthodes de gestion des connexions. Avec la méthode « forkserver », un seul processus est actif en permanence et de nouveaux processus sont créés pour gérer les connexions entrantes. Cette méthode utilise moins de mémoire et réduit légèrement la charge du serveur. La méthode « prefork » garde plusieurs processus inactifs disponibles pour gérer les nouvelles connexions, offrant de meilleures performances au prix de davantage de mémoire utilisée.
";
$elem["qpsmtpd/server_type"]["default"]="forkserver";
...
$elem["qpsmtpd/listen_interfaces"]["descriptionde"]="Adressen, auf denen auf eingehende SMTP-Verbindungen gewartet werden soll:
 ...
 .
 Falls Sie planen, Qpsmtpd zum Zwischenlagern (»spoolen«) von Auslieferungen ferner Rechner an einen lokalen MTA zu verwenden, könnte es in Ihrem Sinne sein, Qpsmtpd auf allen externen Schnittstellen warten zu lassen, während Ihr lokaler MTA auf dem Loopback-Gerät (127.0.0.1) wartet.
 .
 Für jede Schnittstelle können Sie optional einen Port, an dem auf Anfragen gewartet werden soll (statt des Standardports 25), angegeben. Fügen Sie hierzu :<Port> zu der Schnittstelle hinzu, wie in »127.0.0.1:2526«.
";
$elem["qpsmtpd/listen_interfaces"]["descriptionfr"]="Adresses où qpsmtpd sera à l'écoute des connexions SMTP entrantes :
 ...
 .
 Pour chaque interface, le port d'écoute (25 par défaut) peut être modifié en ajoutant :<port> au nom de l'interface. Exemple : « 127.0.0.1:2526 ».
";
...
$elem["qpsmtpd/queue_plugin"]["descriptionfr"]="Méthode de mise en attente pour le courrier accepté :
 ...
 .
 Veuillez choisir « maildir » pour que qpsmtpd place le courrier dans une file d'attente locale au format maildir, plutôt que de le mettre en attente (par exemple si vous installez un piège à pourriel).
 .
...
$elem["qpsmtpd/queue_none_confirm"]["descriptionde"]="Fortfahren ohne Auswahl einer Warteschlangen-Erweiterung?
 Indem Sie »none« für die Warteschlangen-Erweiterung ausgewählt haben, haben Sie das lokale Einstellen in eine Warteschlange für eingehende E-Mail deaktiviert. Damit wird keine E-Mail von Qpsmtpd zwischengelagert, bis Sie manuell eine Warteschlangenmethode konfigurieren. Jeder Rechner, der versucht, E-Mails an Sie zuzustellen, wird bis dahin nicht schwerwiegende 4xx-Fehler erhalten. Damit wird möglicherweise Bandbreite verschwendet und schließlich wird möglicherweise legitime E-Mail zurückgewiesen.
 .
 Um die Warteschlange manuell zu konfigurieren, bearbeiten Sie /etc/qpsmtpd/plugins und wählen Sie eine der dort aufgeführten Warteschlangenmethoden aus. Falls Sie Ihren installierten MTA nicht in der Liste gesehen haben und sich nicht sicher sind, was Sie tun sollen, wählen Sie hier »Abbruch« und stattdessen die SMTP-Proxy-Methode. Konfigurieren Sie Qpsmtpd dann so, dass es Proxy für Ihren lokalen MTA auf einer geeigneten lokalen Adresse/Port.
";
...
";
$elem["qpsmtpd/rcpthosts"]["descriptionde"]="Ziel-Domain(en), für die E-Mail akzeptiert wird (leer für keine):
 Geben Sie eine durch Leerzeichen getrennte Liste von Domain-Namen an, für die Qpsmtpd E-Mail akzeptieren soll. Diese Liste sollte alle Rechner- oder Domain-Namen beinhalten, für die Sie lokale Zustellung akzeptieren wollen, sowie alle Empfänger-Domains, für die Sie als E-Mail-Weiterleitung (»Relay«) auftreten wollen. Falls Sie vorhaben, empfangene E-Mail für einen lokalen MTA zwischenzulagern, sollte diese Liste im Allgemeinen identisch zu der bei diesem MTA verwendeten Liste sein (das Installationsprogramm versucht, diese Einstellung als Voreinstellung zu extrahieren).
 .
...
";
$elem["qpsmtpd/rcpthosts"]["descriptionfr"]="Veuillez choisir un ou plusieurs domaines de destination qui acceptent le courrier. Laissez vide pour aucun.
...
$elem["qpsmtpd/pre094_checkplugin_warning"]["type"]="note";
$elem["qpsmtpd/pre094_checkplugin_warning"]["description"]="Incompatible upstream naming of check_* plugins
