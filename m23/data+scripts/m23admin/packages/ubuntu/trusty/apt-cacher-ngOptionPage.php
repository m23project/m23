...
$elem["apt-cacher-ng/gentargetmode"]["descriptionde"]="Automatische Umleitung der Client-Anfragen:
 Apt-Cacher NG kann Pakete aus anderen Depots beziehen als von denen, die in Client-Aufrufen angegeben wurden. Dadurch kann der Cache effektiver arbeiten, außerdem kann nachträglich einfacher auf anderen Spiegelserver umgestellt werden. Diese URL-Abbildung kann jetzt automatisch eingerichtet werden, unter Bezug des aktuellen Inhalts von /etc/apt/sources.list.
 .
 Bitte auswählen, ob die URL-Abbildung jetzt einmalig konfiguriert werden soll, oder bei jeder Aktuallisierung von Apt-Cacher NG (Konfiguration wird jedes mal angepasst), oder gar nicht konfiguriert werden soll.
 .
...
$elem["apt-cacher-ng/port"]["description"]="Listening TCP port:
 ...
 remains unchanged.
";
$elem["apt-cacher-ng/port"]["descriptionde"]="Annehmender TCP-Port:
 Konfiguriert den TCP-Port, auf dem Apt-Cacher NG die hereinkommenden (Proxy-)HTTP-Anfragen annimmt. Standard ist 3142, es kann auch auf 9999 gesetzt werden, um apt-proxy zu emulieren.
 .
 Eine leerer Eintrag belässt die derzeitige Konfiguration unverändert.
";
...
$elem["apt-cacher-ng/cachedir"]["descriptionde"]="Oberstes Verzeichnis des Paket-Caches:
 Das Hauptverzeichnis des Paket-Caches sollte auf einem Dateisystem ohne Beschränkungen bezüglich Dateinamen liegen.
 .
 Eine leere Eingabe belässt die derzeitige Konfiguration unverändert (gegebenenfalls auf dem Standardverzeichnis /var/cache/apt-cacher-ng ).
";
...
$elem["apt-cacher-ng/proxy"]["descriptionde"]="Proxy für das Herunterladen:
 ...
 .
 Benutzername und Passwort für den Proxy-Server können hier nach dem Muster benutzer:passwort@rechner:port übergeben werden, aber siehe auch Handbuch für Einschränkungen. Ein leerer String hier bedeutet, direkte Verbindungen zu verwenden.
 .
