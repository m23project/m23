...
$elem["tripwire/use-localkey"]["descriptionfr"]="Souhaitez-vous créer ou utiliser votre phrase secrète locale lors de l'installation ?
 Tripwire utilise une paire de clés pour signer différents fichiers, afin de s'assurer de leur intégrité. En acceptant ici, votre phrase secrète vous sera demandée pour la seconde des clés, la clé locale, lors de l'installation. Vous serez également invité à créer une clé pour le site s'il n'en existe pas déjà une. Tripwire utilise la clé locale pour signer les fichiers spécifiques à ce système, par exemple la base de données de Tripwire. Consultez twfiles(5) pour plus d'informations.
 .
 Malheureusement, en raison du processus d'installation de Debian, il existe un intervalle de temps pendant lequel la phrase secrète existe dans un format non-chiffré. Si un attaquant possède l'accès à votre machine pendant ce laps de temps, il pourrait récupérer votre phrase secrète et l'utiliser plus tard.
 .
...
$elem["tripwire/site-passphrase-incorrect"]["descriptionde"]="Ihre Site-Passphrase ist nicht korrekt. Die Operation wiederholen?
 ...
 als root aufrufen.
 .
 Falls Sie Ihre Site-Passphrase vollständig vergessen haben, generieren Sie einen neuen Site-Schlüssel durch Aufruf von
 .
   twadmin -m G -S /etc/tripwire/site.key
 .
 als root aufrufen.
...
$elem["tripwire/local-passphrase-incorrect"]["descriptionde"]="Ihre lokale Passphrase ist nicht korrekt. Die Operation wiederholen?
 ...
 als root aufrufen.
 .
 Falls Sie Ihre lokale Passphrase vollständig vergessen haben, generieren Sie einen neuen Site-Schlüssel durch Aufruf von
 .
   twadmin -m G -L /etc/tripwire/${hostname}-local.key
 .
 als root aufrufen.
...
$elem["tripwire/local-passphrase-incorrect"]["descriptionfr"]="Votre phrase secrète locale est incorrecte. Faut-il recommencer l'opération ?
 ...
 en tant que super-utilisateur (« root »).
 .
 Si vous avez vraiment oublié votre phrase secrète locale, veuillez générer une nouvelle clé de site avec une nouvelle phrase secrète en exécutant :
 .
   twadmin -m G -L /etc/tripwire/${hostname}-local.key
 .
 en tant que super-utilisateur (« root »).
...
$elem["tripwire/change-in-default-policy"]["descriptionfr"]="La règle par défaut de Tripwire a changé
 Depuis la version ${release}, la règle par défaut de Tripwire a changé. Si vous acceptez que dpkg mette à jour /etc/tripwire/twpol.txt et que vous avez précédemment choisi de régénérer automatiquement la règle, la tâche de cron quotidienne de Tripwire échouera tant que vous n'aurez pas mis à jour votre base de données Tripwire.
 .
