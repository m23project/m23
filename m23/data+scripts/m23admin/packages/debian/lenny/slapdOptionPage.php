...
$elem["slapd/invalid_config"]["descriptionde"]="Konfiguration erneut versuchen?
 Die von Ihnen eingegebene Konfiguration ist ungültig. Stellen Sie sicher, dass der DNS-Domainname einer gültigen Syntax folgt, die Organisation nicht leer geblieben ist und dass die Administratorpasswörter übereinstimmen. Falls Sie sich entscheiden, die Konfiguration nicht erneut zu versuchen, wird der LDAP-Server nicht eingerichtet. Führen Sie »dpkg-reconfigure slapd« aus, falls Sie die Konfiguration später erneut versuchen wollen.
";
$elem["slapd/invalid_config"]["descriptionfr"]="Faut-il recommencer la configuration ?
 La configuration que vous avez indiquée n'est pas valable. Veuillez vérifier que le nom de domaine DNS utilise une syntaxe correcte, que le champ « organisation » n'est pas vide et que les mots de passe d'administrateur correspondent. Si vous choisissez de ne pas recommencer la configuration, le serveur LDAP ne sera pas configuré. Si vous voulez recommencer ce processus, utilisez la commande « dpkg-reconfigure slapd ».
";
...
$elem["slapd/suffix_change"]["type"]="boolean";
$elem["slapd/suffix_change"]["description"]="Back up current database and create a new one?
...
$elem["slapd/slurpd_obsolete"]["type"]="error";
$elem["slapd/slurpd_obsolete"]["description"]="slurpd is obsolete; replicas must be reconfigured by hand
 ...
 details.
";
$elem["slapd/slurpd_obsolete"]["descriptionde"]="Slurpd ist veraltet; Replikas müssen von Hand rekonfiguriert werden
 In Ihrer Konfiguration wurde beim Upgrade eine oder mehrere »replica«-Optionen gefunden. Da slurpd beginnend mit OpenLDAP 2.4 veraltet ist, müssen Sie Ihre Repliken auf die Verwendung des Syncrepl-Protokolls migrieren.
 .
 Die Umstellung von slurpd auf das »pull«-basierte Syncrepl-Protokoll kann nicht automatisch geschehen und Sie müssen Ihre Repliken-Server von Hand konfigurieren. Bitte lesen Sie http://www.openldap.org/doc/admin24/syncrepl.html für Details.
";
$elem["slapd/slurpd_obsolete"]["descriptionfr"]="reconfiguration manuelle des réplicats
 Une ou plusieurs options « replica » de slurpd ont été trouvée dans le fichier de configuration lors de la mise à niveau. Le programme slurpd est obsolète à partir de la version 2.4 d'OpenLDAP et il est nécessaire de migrer les réplicats pour qu'ils utilisent le protocole syncrepl à la place.
 .
 Cette conversion ne peut se faire automatiquement et vous devez configurer les serveurs réplicats vous-même. Veuillez consulter http://www.openldap.org/doc/admin24/syncrepl.html pour plus d'informations.
";
$elem["slapd/slurpd_obsolete"]["default"]="";
$elem["slapd/tlsciphersuite"]["type"]="error";
$elem["slapd/tlsciphersuite"]["description"]="TLSCipherSuite values have changed
...
";
$elem["slapd/tlsciphersuite"]["descriptionde"]="TLSCipherSuite-Werte haben sich geändert
 Eine Option »TLSCipherSuite« wurde beim Upgrade in Ihrer Slapd-Konfiguration gefunden. Die erlaubten Werte hierfür hängen von der verwendeten SSL-Implementation ab, die von OpenSSL auf GnuTLS geändert wurde. Im Ergebnis werden Ihre existierenden TLSCipherSuite-Einstellungen nicht mit diesem Paket funktionieren.
 .
 Diese Einstellung wurde für Sie automatisch auskommentiert. Falls Sie spezielle Anforderung an die Verschlüsselung haben, bei denen diese Option wieder aktiviert werden muss, lesen Sie die Ausgabe von »gnutls-cli -l« aus dem Paket Gnutls-bin für die Liste der von GnuTLS unterstützen Chiffren.
";
$elem["slapd/tlsciphersuite"]["descriptionfr"]="Changement des valeurs possibles pour « TLSCipherSuite »
 L'option « TLSCipherSuite » a été trouvée dans le fichier de configuration de slapd lors de la mise à niveau. Les valeurs possibles pour cette option dépendent de l'implémentation de SSL qui est utilisée. Comme OpenSSL a été remplacé par GnuTLS, les réglages actuels de « TLSCipherSuite » ne fonctionnent plus avec cette version du paquet.
 .
 Ce réglage a été automatiquement mis en commentaire. Si une méthode spécifique de chiffrement impose de la réactiver, vous devriez consulter l'affichage de la commande « gnutls-cli -l » du paquet gnutls-bin pour une liste des méthodes de chiffrement gérées par GnuTLS.
";
$elem["slapd/tlsciphersuite"]["default"]="";
