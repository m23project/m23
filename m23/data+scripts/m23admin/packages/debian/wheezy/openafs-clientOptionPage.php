...
";
$elem["openafs-client/cell-info"]["descriptionde"]="Rechnername des Datenbank-Servers für Ihre Home-Zelle:
...
$elem["openafs-client/run-client"]["descriptionde"]="Soll der Openafs-Client jetzt und beim Booten gestartet werden?
 Normalerweise erwarten Benutzer, die das Paket openafs-client installieren, dass AFS automatisch beim Boot eingehängt wird. Falls Sie aber eine neue Zelle erstellen wollen, oder einen Laptop benutzen, möchten Sie es vielleicht nicht beim Boot gestartet haben. Wenn Sie sich dafür entscheiden, AFS nicht beim Rechnerstart zu starten, können Sie /etc/init.d/openafs-client force-start ausführen, um den AFS-Client zu starten.
";
...
$elem["openafs-client/crypt"]["descriptionde"]="Soll authentifizierter Verkehr mit AFS-Dateiservern verschlüsselt werden?
 AFS bietet eine schwache Form der Verschlüsselung, die optional zwischen Client und Dateiserver benutzt werden kann. Obwohl diese Verschlüsselung schwächer als DES und daher nicht ausreichend für sehr vertrauliche Daten ist, macht es einem möglichen Angreifer die Arbeit dennoch deutlich schwerer.
";
...
$elem["openafs-client/dynroot"]["descriptionde"]="Soll der Inhalt von /afs dynamisch generiert werden?
 ...
 .
 Wenn Sie /afs dynamisch erstellen lassen, werden Sie vielleicht /etc/openafs/CellAlias erstellen müssen, sodass die Datei Aliase für gebräuchliche Zellen enthält. Die Syntax ist: Eine Zeile pro Aliase, mit Zellenname - Leerzeichen - Alias.
";
...
$elem["openafs-client/fakestat"]["descriptionde"]="Soll fakestat benutzt werden, um eine Verzögerung beim Auflisten von /afs zu vermeiden?
 Da AFS ein globaler Dateiraum ist, können Operationen im /afs-Verzeichnis zu großen Netzverkehr führen. Wenn einige AFS-Zellen nicht erreichbar sind, kann das Anschauen von /afs (mit ls oder einem Dateibrowser) zu minutenlangem Aufhängen führen. AFS kann aber Antworten zu solchen Operationen simulieren, um ein Aufhängen zu vermeiden. Die meisten Leute wollen das.
";
