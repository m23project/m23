...
$elem["freepops/init"]["descriptionde"]="Freepopsd automatisch nach jedem Systemstart starten?
 Der FreePOPs-Daemon kann nach jedem Systemstart automatisch gestartet werden. Standardmäßig wird er mit Port 2000 verknüpft. Diese Einstellung kann durch Bearbeiten der Datei /etc/default/freepops geändert werden.
";
...
";
$elem["freepops/jail"]["descriptionde"]="Ein Chroot-Jail (abgeschlossene Umgebung) für FreePOPs erstellen?
 FreePOPs kann in einer Chroot-Umgebung gestartet werden, um die Sicherheit des Systems zu verbessern.
 .
 Die abgeschlossene Umgebung (Jail) wird in /var/lib/freepops/chroot-jail/ erstellt. Das reguläre Init-Skript wird sich darum kümmern, den Daemon zu starten, indem ein Skript namens start.sh im Wurzelverzeichnis des Chroot-Jails gestartet wird.
";
...
$elem["freepops/updates"]["descriptionde"]="Lokale Aktualisierungen beim Upgrade entfernen?
 ...
 .
 Diese Aktualisierungen werden typischerweise in zukünftige FreePOPs-Veröffentlichungen integriert oder können durch Aufruf des Aktualisierungsprogramms erneut heruntergeladen werden. Daher können sie ohne Probleme entfernt werden, wenn ein Upgrade des Pakets durchgeführt wird.
 .
 Die unter /var/lib/freepops/lua_updates gespeicherten Module haben eine höhere Priorität als die Module aus /usr/share/freepops/lua. Es wird daher empfohlen, erstere zu entfernen, um zu vermeiden, dass veraltete Module verwendet werden, es sei denn, Sie möchten lokale Änderungen unabhängig von Upgrades von FreePOPs einfrieren.
";
