...
$params = PKG_OptionPageHeader2("firebird2.1-classic");
...
";
$elem["shared/firebird/sysdba_password/first_install"]["descriptionfr"]="Mot de passe pour l'identifiant SYSBDA :
 Firebird crée un identifiant spécial appelé SYSBDA, qui a accès à toutes les bases de données. SYSBDA peut également créer de nouvelles bases de données et de nouveaux utilisateurs. Par sécurité, il est nécessaire d'attribuer un mot de passe à SYSBDA.
 .
 ...
 .
 Si vous laissez ce champ vide, un mot de passe aléatoire sera créé et écrit dans SYSBDA.password.
";
...
";
$elem["shared/firebird/sysdba_password/upgrade_reconfigure"]["descriptionfr"]="Mot de passe pour l'identifiant SYSBDA :
 Firebird crée un identifiant spécial appelé SYSBDA, qui a accès à toutes les bases de données. SYSBDA peut également créer de nouvelles bases de données et de nouveaux utilisateurs. Par sécurité, il est nécessaire d'attribuer un mot de passe à SYSBDA.
 .
 Le mot de passe est conservé dans le fichier /etc/firebird/${FB_VER}/SYSBDA.password (accessible uniquement au superutilisateur). Pour changer de mot de passe, vous pouvez modifier directement ce fichier (dans ce cas n'oubliez pas de mettre également à jour la base de données « security » avec l'utilitaire gsec), ou bien utiliser la commande « dpkg-reconfigure ».
 .
...
$elem["shared/firebird/purge_security"]["descriptionde"]="Passwort-Datenbank löschen?
 Das letzte Paket, dass die Passwort-Datenbank unter /var/lib/firebird/${FB_VER}/system/security.fdb nutzt, wird gerade komplett entfernt.
 .
...
$elem["shared/firebird/purge_security"]["descriptionfr"]="Faut-il supprimer le mot de passe de la base de données ?
 Le dernier paquet à utiliser le mot de passe de la base de données dans /var/lib/firebird/${FB_VER}/system/security.fdb va être purgé.
 .
