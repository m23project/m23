...
$elem["uprecords-cgi/layout"]["descriptionde"]="Format zur Verwendung durch uprecords.cgi:
 Es sind verschiedene Layouts für die Verwendung durch das uprecords-CGI-Skript verfügbar. Welches Sie verwenden, ist Geschmackssache.
 .
...
$elem["uprecords-cgi/maxentries"]["descriptionde"]="Anzahl der Aufzeichnungen, die von uprecords.cgi angezeigt werden:
 Uptimed sammelt unter Umständen viele Uptime-Aufzeichnungen, die nicht alle für jedermann interessant sind. Hier können Sie einstellen, wieviele Aufzeichnungen angezeigt werden.
";
...
$elem["uprecords-cgi/install_note"]["descriptionde"]="uprecords.cgi wurde im Web tree installiert
 Sie haben gerade das Paket »uprecords-cgi« installiert. Das bedeutet, dass ein neues CGI-Skript installiert wurde, welches von außerhalb als http://${hostname}/cgi-bin/uprecords.cgi verfügbar ist (wenn Sie die Web-Server-Konfiguration nicht geändert haben, so dass CGI-Skripte in einem anderen Pfad liegen).
 .
 In der Standard-Konfiguration des Web-Servers sind CGI-Skripte weltweit abrufbar. Wenn Sie das nicht möchten, sollten Sie entsprechende Beschränkungen einrichten.
 .
...
$elem["uprecords-cgi/install_note"]["descriptionfr"]="uprecords.cgi installé dans l'arborescence du serveur web
 ...
 .
 Dans la configuration par défaut du serveur web, les scripts CGI sont accessibles sans restriction. Si vous ne le souhaitez pas, vous devriez définir des accès restreints.
 .
